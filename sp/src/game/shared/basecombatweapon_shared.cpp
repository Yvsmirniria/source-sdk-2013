...
{
#if !defined( CLIENT_DLL )
	if (Q_strcmp( STRING( gpGlobals->mapname ), "mob_melee_tut" ) == 0)
		return false;
#endif
...
bool CBaseCombatWeapon::DefaultDeploy( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
	...
	float delay = MIN( SequenceDuration(), 0.2f );;
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner )
	{
		...
		if (pOwner->m_nMeleeState > MELEE_NO)
		{
			delay = 0.0f;
		}
		pOwner->SetNextAttack( gpGlobals->curtime + delay );
	}
...
void CBaseCombatWeapon::ItemPreFrame( void )
{
	...
	if (m_flNextPrimaryAttack <= gpGlobals->curtime)
	{
		...
		if (pPlayer->m_nMeleeState == MELEE_DONE) {
			// MELEE_DONE means speed up the draw animation.
			// If we can attack again, that means the animation is done
			pPlayer->m_nMeleeState = MELEE_NO;
		}
	}
...
float CBaseCombatWeapon::GetPlaybackRate()
{
	CBasePlayer* pPlayer = ToBasePlayer( GetOwner() );
	if (pPlayer && pPlayer->m_nMeleeState == MELEE_DONE) {
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalActiveWeaponData )
	...
	SendPropTime( SENDINFO( m_flLastHitTime ) ),
	SendPropInt( SENDINFO( m_nPelletHits ) ),
	SendPropArray( SendPropVector( SENDINFO_ARRAY( m_vecPelletXYZs ), -1, SPROP_COORD ), m_vecPelletXYZs ),
#if defined( TF_DLL )
	...
	RecvPropTime( RECVINFO( m_flLastHitTime ) ),
	RecvPropInt( RECVINFO( m_nPelletHits ) ),
	RecvPropArray( RecvPropVector( RECVINFO_ARRAY( m_vecPelletXYZs[0] ) ), m_vecPelletXYZs ),
#endif
END_NETWORK_TABLE()
