...
public:
	...
	virtual float           GetPlaybackRate( void );
...
// Client only methods
	...
	virtual bool            CrouchingImprovesAccuracy() { return false; }
...
public:
	...
	CNetworkVar( float, m_flLastHitTime ); // last time weapon hit an enemy (for hud hit markers)
	CNetworkVar( int, m_nPelletHits ); // num shotgun pellets that hit
	CNetworkArray( Vector, m_vecPelletXYZs, MAX_PELLET_MARKS );
