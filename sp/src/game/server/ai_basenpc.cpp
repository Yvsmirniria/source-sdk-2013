...
{
	if (start.IsValid() && end.IsValid() && dir.IsValid())
	{
		Ray_t ray;
		ray.Init( start, end );
	...
	{
		DevMsg( "TraceAttackToTriggers() - ignoring invalid vector\n" );
	}
}
...
void CBaseEntity::HandleShotImpactingGlass( const FireBulletsInfo_t &info, 
	...
	bool bGlassFx /*=true*/ )
{
	// Also using this function to allow the 357 to penetrate anything.
	// Change the behaviour in that case
...
bool CAI_BaseNPC::CanPlantHere( const Vector &vecPos )
{
...
void CAI_BaseNPC::TestPlayerPushing( CBaseEntity *pEntity, bool bForce )
{
